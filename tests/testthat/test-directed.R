...
test_that("grangerCausality returns correct structure", {
  ...
  expect_type(result, "list")
  expect_named(result, c("gc_xy", "gc_yx", "net_gc", "order", "method"))
  expect_identical(result$method, "parametric")
...
test_that("grangerCausality selects nonparametric without changing parametric", {
  set.seed(3)
  sigs <- make_directed_signals(n = 2000, sr = 500, lag_samples = 5,
                                coupling = 0.5)
  nonparametric <- grangerCausality(
    sigs$x, sigs$y, sr = sigs$sr, method = "nonparametric"
  )
  expect_identical(nonparametric$method, "nonparametric")
  expect_gt(nonparametric$gc_xy, nonparametric$gc_yx)

  # parametric path still works and is tagged
  res <- grangerCausality(sigs$x, sigs$y, sr = sigs$sr, order = 5L,
                          method = "parametric")
  expect_identical(res$method, "parametric")
})
