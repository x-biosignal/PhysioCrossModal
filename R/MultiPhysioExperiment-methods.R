...
# ---- couplingResults cache accessors -------------------------------------

#' Cached coupling results on a MultiPhysioExperiment
#'
#' Get or set the list of cached coupling-analysis results stored on a
#' \code{\link{MultiPhysioExperiment}}. The cache is populated by
#' \code{\link{couplingAnalysisCached}()}, which keys entries by a digest of the
#' analysis arguments so repeated identical requests are not recomputed.
#'
#' @param x A \code{MultiPhysioExperiment} object.
#' @param value A named list of cached results.
#' @return \code{couplingResults()} returns the cache list; the replacement form
#'   returns the updated object.
#' @seealso [couplingAnalysisCached()]
#' @export
setGeneric("couplingResults", function(x) standardGeneric("couplingResults"))

#' @rdname couplingResults
setMethod("couplingResults", "MultiPhysioExperiment", function(x) x@couplingResults)

#' @rdname couplingResults
#' @export
setGeneric("couplingResults<-", function(x, value) standardGeneric("couplingResults<-"))

#' @rdname couplingResults
setReplaceMethod("couplingResults", "MultiPhysioExperiment", function(x, value) {
  stopifnot(is.list(value))
  x@couplingResults <- value
  validObject(x)
  x
})
