...
#'
#' Non-iterative amplitude-adjusted surrogate (Theiler et al., 1992): map the
#' series to a Gaussian ordering, phase-randomize, then map back to the original
#' value distribution. Approximately preserves both the power spectrum and the
#' amplitude distribution while destroying nonlinear / cross-signal structure.
#'
#' @param x Numeric vector.
#' @return Numeric surrogate vector (a permutation of \code{x}).
#' @noRd
.aaft_surrogate <- function(x) {
  n <- length(x)
  # Gaussian series with the same rank order as x
  g <- sort(stats::rnorm(n))[rank(x, ties.method = "first")]
  g_surr <- .phase_randomize_surrogate(g)
  # Reorder the original values by the ranks of the phase-randomized Gaussian
  sort(x)[rank(g_surr, ties.method = "first")]
}

#' Iterative amplitude-adjusted Fourier transform (IAAFT) surrogate
#'
#' Generates a surrogate that preserves both the power spectrum (amplitude of
#' the Fourier transform) and the value distribution of \code{x}, following
#' Schreiber & Schmitz (1996). It alternates between (a) replacing the current
#' surrogate's amplitude spectrum with the target spectrum (keeping its phases)
#' and (b) rank-ordering it back onto the sorted target values, until the rank
#' assignment converges (or \code{n_iter} is reached). The returned surrogate
#' ends on the amplitude-adjustment step, so its value distribution is exactly
#' that of \code{x}.
#'
#' @param x Numeric vector.
#' @param n_iter Maximum number of iterations (default: 100).
#' @param tol Convergence tolerance on the relative spectral change; iteration
#'   also stops when the rank assignment no longer changes (default: 1e-8).
#' @return Numeric surrogate vector (a permutation of \code{x}).
#' @references
#' Schreiber, T., & Schmitz, A. (1996). "Improved surrogate data for
#' nonlinearity tests." \emph{Physical Review Letters}, 77(4), 635-638.
#' \doi{10.1103/PhysRevLett.77.635}
#' @noRd
.iaaft_surrogate <- function(x, n_iter = 100, tol = 1e-8) {
  n <- length(x)
  sorted_x <- sort(x)
  target_amp <- Mod(stats::fft(x))

  s <- sample(x)                                   # random shuffle start
  prev_rank <- rank(s, ties.method = "first")
  prev_err <- Inf

  for (iter in seq_len(n_iter)) {
    # (a) impose the target amplitude spectrum, keep the current phases
    phases <- Arg(stats::fft(s))
    s1 <- Re(stats::fft(target_amp * exp(complex(imaginary = phases)),
                        inverse = TRUE)) / n

    # (b) impose the target value distribution via rank ordering
    r <- rank(s1, ties.method = "first")
    s2 <- sorted_x[r]

    # convergence: rank assignment unchanged, or spectral error plateaued
    err <- mean(abs(Mod(stats::fft(s2)) - target_amp)) / mean(target_amp)
    s <- s2
    if (identical(r, prev_rank) || abs(prev_err - err) < tol) break
    prev_rank <- r
    prev_err <- err
  }
  s
...
#'   \code{"phase"} (default; Fourier phase randomization), \code{"timeshift"}
#'   (circular time shift), \code{"iaaft"} (iterative amplitude-adjusted Fourier
#'   transform, preserving the power spectrum and value distribution; Schreiber &
#'   Schmitz, 1996), or \code{"aaft"} (non-iterative amplitude-adjusted FT).
...
.dispatch_coupling <- function(sig_x, sig_y, sr, method, ...) {
  switch(method,
    ...
    pac = phaseAmplitudeCoupling(sig_x, sig_y, sr = sr, ...),
