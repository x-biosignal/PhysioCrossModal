...
elasticAlign <- function(x, y, t = NULL, lambda = 0, method = c("srvf"),
  ...
  if (use_fd) {
    ...
    # fdasrvf renamed the aligned-function field f2n -> f2tilde (>= 2.x); accept
    # either so the aligned curve keeps the grid length rather than becoming
    # empty (an empty aligned curve breaks srvfMean's length invariant).
    aligned <- as.numeric(if (!is.null(fit$f2tilde)) fit$f2tilde else fit$f2n)
    solver <- "fdasrvf"
  } else {
