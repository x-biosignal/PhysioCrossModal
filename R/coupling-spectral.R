...
#'   future use.
#' @param type Character coherence type: \code{"magnitude"} (magnitude-squared
#'   coherence, the default), \code{"imaginary"} (the imaginary part of
#'   coherency, insensitive to zero-lag/volume-conduction coupling; Nolte et
#'   al., 2004), or \code{"lagged"} (Pascual-Marqui, 2007:
#'   \code{Im(C) / sqrt(1 - Re(C)^2)} with \code{C} the complex coherency).
...
#'   \describe{
#'     \item{coherence}{Numeric vector of coherence values at each frequency:
#'       magnitude-squared coherence in \eqn{[0, 1]} for
#'       \code{type = "magnitude"}, or the (signed) imaginary / lagged coherence
#'       for the other types.}
#'     \item{type}{The coherence type computed.}
...
#' using the imaginary part of coherency. \emph{Clinical Neurophysiology},
...
#' \emph{arXiv:0711.1455}.
#'
#' @seealso [crossSpectrum()], [multitaperCoherence()], [laggedCoherence()],
...
coherence <- function(x, y = NULL, sr = NULL, freq_range = NULL,
  ...
  # Complex coherency C(f) = Sxy(f) / sqrt(Sxx(f) * Syy(f)); the one-sided
  # doubling of csd and psd cancels here just as it does for the magnitude.
  coherency <- csd_vals / sqrt(psd_xx$psd * psd_yy$psd)

  coh <- switch(type,
    # magnitude-squared coherence |C|^2 = |Sxy|^2 / (Sxx * Syy), clamped to [0, 1]
    magnitude = pmin(pmax(Mod(coherency)^2, 0), 1),
    # imaginary part of coherency (Nolte et al., 2004)
    imaginary = Im(coherency),
    # lagged coherence (Pascual-Marqui, 2007): Im(C) / sqrt(1 - Re(C)^2)
    lagged = Im(coherency) /
      sqrt(pmax(1 - Re(coherency)^2, .Machine$double.eps))
  )
...
#'
#' Convenience wrapper for \code{\link{coherence}} with \code{type = "lagged"},
#' the lagged (non-instantaneous) coherence of Pascual-Marqui (2007),
#' \code{Im(C) / sqrt(1 - Re(C)^2)} where \code{C} is the complex coherency. Like
#' the imaginary part of coherency it is insensitive to zero-lag /
#' volume-conduction coupling.
#'
#' @inheritParams coherence
#' @return A named list as returned by \code{\link{coherence}}, with
#'   \code{type = "lagged"}.
#' @references
#' Pascual-Marqui, R. D. (2007). Instantaneous and lagged measurements of linear
#' and nonlinear dependence between groups of multivariate time series.
#' \emph{arXiv:0711.1455}.
#' @seealso \code{\link{coherence}}
#' @export
#' @examples
#' sr <- 200
#' t <- seq(0, 10, length.out = sr * 10)
...
laggedCoherence <- function(x, y = NULL, sr = NULL, freq_range = NULL,
                            nperseg = 256L, noverlap = NULL,
                            method = c("welch", "multitaper"),
                            modality_x = NULL, modality_y = NULL,
                            channels_x = 1L, channels_y = 1L) {
  coherence(x, y = y, sr = sr, freq_range = freq_range, nperseg = nperseg,
            noverlap = noverlap, method = method, type = "lagged",
            modality_x = modality_x, modality_y = modality_y,
            channels_x = channels_x, channels_y = channels_y)
}
