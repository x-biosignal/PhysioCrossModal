...
weightedPLI <- function(x, y = NULL, sr = NULL, freq_band, debiased = TRUE,
  ...
  # wPLI and debiased wPLI (Vinck et al. 2011, NeuroImage 55:1548-1565, Eq. 6)
  # via the shared PhysioCore estimator, so this matches PhysioEEG::eegWPLI.
  est <- PhysioCore::wpliEstimate(imag_cross, debiased = debiased)

  list(
    wpli = est$wpli,
    wpli_debiased = if (debiased) est$wpli_debiased else NULL,
