...
#'   (default: the dark end of the colorblind-safe sequential palette,
#'   \code{PhysioCore::physioPalette(2, "sequential")[1]}).
...
plotCouplingMatrix <- function(mat, title = "Coupling Matrix",
                               low_colour = "white",
                               high_colour = PhysioCore::physioPalette(
                                 2, "sequential")[1], ...) {
...
plotCoherenceSpectrum <- function(result, show_threshold = TRUE,
  ...
  coh_type <- if (!is.null(result$type)) result$type else "magnitude"
  ylab <- switch(coh_type,
    magnitude = "Magnitude-squared coherence",
    imaginary = "Imaginary coherence",
    lagged = "Lagged coherence",
    "Coherence")
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x = Frequency, y = Coherence)) +
    ...
    ggplot2::labs(title = title, x = "Frequency (Hz)", y = ylab) +
    ggplot2::theme_minimal()

  if (coh_type == "magnitude") {
    # magnitude-squared coherence is in [0, 1]; the confidence limit applies
    ...
    # imaginary / lagged coherence is signed; mark the zero line
    p <- p + ggplot2::geom_hline(yintercept = 0, colour = "grey60",
                                 linewidth = 0.3)
  }
